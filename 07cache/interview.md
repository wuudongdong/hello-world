...
## 11. Redis 分布式锁的续命问题？
可以使用 Redission 来实现分布式锁，它有关锁续命是使用看门狗，就是一个后台线程，每隔一段时间检查锁是否还在，在的话就延长锁的过期时间。

## 12. Redis 变慢了有哪些排查的思路？
首先看是否有使用时间复杂度高的指令，像keys、smembers、排序、做交叉并集，建议使用复杂度低的指令代替复杂度高的指令，比如使用scan多次代替smembers，
在客户端完成排序和交叉并集的操作；
其次看是否给多个key设置了相同时间的过期时间，Redis 的周期性的删除 key 会导致阻塞主线程；
其次看Redis 的 AOF 的同步策略，是否会产生 fsync 与 aof 重写进程竞争磁盘 io 的情况；
使用 cat smaps 指令查看是否由于内存不足产生了磁盘 swap；